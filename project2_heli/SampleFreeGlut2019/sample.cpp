...
GLuint  heliList;				// creates the helicopter
GLuint  bladeList;				//creates the rotors
GLuint  diamondList;
...
float BladeAngle;
float BladeAngle_TailRotor;
float pyramidAngle;
int cockpit = 0; // 1 if we are in cockpit view, 0 if we are not
...
Display( )
{
	...
	if (cockpit != 0) {
		...
		Xrot = 0;
		Yrot = 0;//DISABLE rotations while in cockpit view
	...
	glPushMatrix();
		...
		glScalef(3., 3., 3.);
		glRotatef(BladeAngle_TailRotor, 1., 0., 0.);
		glRotatef(90., 0., 1., 0.);
		glColor3f(1., 1., 1.);
		glCallList(bladeList); //calls blade list for the small rotor and applies the appropriate transformations.
	glPopMatrix();
	...
	glPushMatrix();
		...
		glRotatef(90., 1., 0., 0.);
		glColor3f(1., 1., 1.);
		glCallList(bladeList); //calls blade list for the big rotor and applies the appropriate transformations.
	glPopMatrix();
	...
	glLoadIdentity( );
	glColor3f( 1., 1., 1. );
	DoRasterString( 5., 5., 0., (char *)"Animated Helicopter" );
...
InitLists( )
{
	...
	glBegin(GL_TRIANGLES);
		glColor3f(1., 0., 0.);
		 ...
		 glVertex3f(-2.0, -2.0, 2.0);
		 glVertex3f(0.0, -2.0, -2.0);
		 glVertex3f(2.0, -2.0, 2.0);// bottom face
